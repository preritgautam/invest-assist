  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (contentType.includes('application/json')) {
        ...
        console.error('[REX API] Result fetch error:', errorData);
        return NextResponse.json(
        ...
        console.error('[REX API] Result fetch error (non-JSON):', errorText.substring(0, 200));
        return NextResponse.json(
    ...
    if (!contentType.includes('application/json')) {
      ...
      console.error('[REX API] Result fetch returned non-JSON despite 200 OK:', responseText.substring(0, 200));
      return NextResponse.json(
