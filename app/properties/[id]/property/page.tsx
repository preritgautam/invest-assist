...
import { PropertyDocument } from "@/components/tabs/docs-tab/types"
...
export default function PropertyPage({ params }: PropertyPageProps) {
    ...
    useEffect(() => {
      async function fetchDocuments() {
        try {
          ...
          const url = id
            ? `/api/documents/list?propertyId=${encodeURIComponent(id)}`
            : '/api/documents/list'
          ...
          if (data.documents && data.documents.length > 0) {
            ...
            const completedDocuments = data.documents.filter((doc: DatabaseDocument) => doc.extraction_status === "completed")
  ...
  return (
    <PropertyLayoutWrapper propertyId={id} currentTab="property">
      <PropertyTab property={property} documents={documents} />
